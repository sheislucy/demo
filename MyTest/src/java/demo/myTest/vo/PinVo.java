...
import demo.myTest.domains.Activity;
import demo.myTest.domains.CityMeta;
import demo.myTest.domains.Comment;
import demo.myTest.domains.Spot;
...
public class PinVo implements Serializable {
	...
	public static PinVo from(Spot spot, CityMeta cityMeta, Activity act,
			Page<Comment> cmts) {
		if (spot == null)
			return null;
		PinVo vo = new PinVo();
		...
		if (spot.getPlace() != null) {
			...
			vo.setPlaceAddr(new StringBuilder()
					...
					.append(spot.getPlace().getStreet()).toString());
		} else if (cityMeta != null) {
			vo.setPlaceAddr(cityMeta.getName());
			vo.setCity(cityMeta.getPinyin());
		}
		if (spot.getImage() != null) {
			vo.setImageUrl(new StringBuilder().append(ApplicationConfig.base)
					.append(ApplicationConfig.imageRefer).append("/")
					.append(spot.getImage().getResId()).toString());
			if (spot.getImage().getOrgSize() != null
					&& spot.getImage().getOrgSize().length == 2) {
				vo.setImageHeight(Math.round(192
						* spot.getImage().getOrgSize()[0]
		...
		if (spot.getCreatedBy() != null) {
			...
			vo.setCreatedByAvatarUrl("/img/avatar-female.jpg");
		}
		if (cmts != null) {
